...
import { createBidEvent, acceptBidEvent, eventToNotification } from '../lib/logging'
...
const acceptBid = async (io, mission, bid, acceptingBidderId, currentUserUid) => {
...
export default () => {
  ...
  api.post('/:bidId/accept', swcAuthenticatedMiddleware, celebrate({
    ...
    await acceptBid(req.io, mission, bid, req.body.bidderId, req.swcUid)
    res.status(200).send()
  })
